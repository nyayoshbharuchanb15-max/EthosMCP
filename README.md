...
**Status:** Functional Prototype & Open-Source Compliance Methodology
...
This repository now contains a **functional prototype** of an MCP server, serving as a **vendor-neutral architectural specification and audit methodology** for engineering and compliance teams to collaborate on automated, zero-trust ethical AI governance. 
...
## Running the EthosMCP Server (Functional Prototype)

To run the functional prototype of the EthosMCP server, follow these steps:
...
2.  **Set up environment variables:**
    ...
    ```dotenv
    PORT=8000
    DATABASE_URL="postgresql://user:password@db:5432/ethosmcp_db" # Placeholder, not actively used in this prototype
    CRYPTO_KEY="your_strong_cryptographic_key_here" # IMPORTANT: Replace with a strong, random key
    ```
...
4.  **Access the MCP server:**
    The server will be running at `http://localhost:8000`. You can interact with it using an MCP client or by sending HTTP requests to the defined endpoints (e.g., `/governance/audit_ropa_alignment`).

    **Example API Endpoints (using `curl`):**

    *   **Audit ROPA Alignment:**
        ```bash
        curl -X POST http://localhost:8000/governance/audit_ropa_alignment
        ```
    ...
    *   **Query Consent Registry:**
        ```bash
        curl -X POST http://localhost:8000/sovereignty/query_consent_registry
        ```

    *   **Simulate DSAR Workflow (example with erasure latency):**
        ```bash
        curl -X POST -H "Content-Type: application/json" -d '{"user_id": "user_001", "request_type": "erasure", "erasure_latency_days": 25}' http://localhost:8000/sovereignty/simulate_dsar_workflow
        ```

    *   **Audit Encryption Coverage:**
        ```bash
        curl -X POST http://localhost:8000/security/audit_encryption_coverage
        ```

This functional prototype demonstrates the core audit capabilities of EthosMCP. Further development will involve integrating with real data sources and enhancing the audit logic.
